...
| 2026-07-20 | 챗봇(우측 하단 위젯) 신규 추가 — 특정 담당자 소유가 아닌 공통 영역(레이아웃/파셜/공용 css·js/`app.js`/DB 스크립트/`package.json`)에 걸쳐 있음. 신규 파일: `src/routes/chatbot.js`, `src/controllers/chatbotController.js`, `src/models/chatbotModel.js`, `src/views/partials/chatbot.ejs`, `public/js/common/chatbot.js`, `public/css/chatbot.css`, `db/scripts/42_create_chatbot_messages_table.sql`. 상세는 `docs/chatbot.md`. | (요청자) |
| 2026-07-22 | 박승욱이 원 담당(임가윤) 승인 하에 `src/views/admin/api-form.ejs`(엔드포인트 URL 입력칸 placeholder + SVC ID/REST 경로에 따른 🧪 테스트 버튼 노출 여부 안내 문구)를 예외적으로 직접 수정. 대형법인사(콘도)/골프장 키오스크 API Reference 화면 UI를 맞추는 작업(`db/scripts/44_update_bigcorp_resort_reservation_api.sql` 실행 포함)의 일부. | 박승욱 |
| 2026-07-24 | 박승욱이 원 담당(임가윤) 승인 하에 `src/controllers/adminController.js`(`/admin` 기본 리다이렉트를 `/admin/partners` → `/admin/apis`로 변경)와 `src/views/admin/api-form.ejs`(저장/등록 버튼 영역에 `margin-bottom` 추가 — 우측 하단 챗봇 위젯 아이콘에 가려 클릭이 어렵던 문제 수정)를 예외적으로 직접 수정. | 박승욱 |
| 2026-07-24 | 박승욱이 원 담당(윤태윤) 승인 하에 `src/controllers/homeController.js`(홈 화면 "API 명세" 통계에서 `category='common'`(에러 코드 등, 실제 연동 API 정의가 아닌 참고 정보) 제외)를 예외적으로 직접 수정. | 박승욱 |
